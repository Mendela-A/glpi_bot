...
Тести живуть у `bot/tests/`. Запуск:
```bash
docker compose run --rm bot python3 -m pytest tests/ -v
```

**Правило:** після додавання або зміни функціоналу — перевіряй, чи потрібен тест.
Якщо зачеплена логіка, що вже покрита тестами (`test_polling.py`, `test_middleware.py` тощо) — оновлюй відповідний файл.
Якщо логіка нова й нетривіальна — додавай тест у відповідний `test_*.py`.
